...
function DiagonalVideo({ children }) {
  ...
  const diagonalPadding = () => {
    ...
    setPadding((Math.tan(radians) * innerWidth) / 2);
  ...
  const mobileDiagonalHeight = () => {
    ...
    const { height } = videoElement.getBoundingClientRect();
  ...
  const updateVideoHeight = () => {
    const diagonalElement = diagonal.current as HTMLElement;
    if (padding > 0) {
      if (window.innerWidth > 1000) diagonalElement.style.height = '90vh';
  ...
  const exitFullscreen = (exit) => {
    ...
    anchorElement.click();
    if (exit === true) setTimeout(() => videoControl(), 200);
  };
  ...
  return (
    <SectionStyled>
      ...
      >
        ...
        <TitleStyled
          style={{ display: isPlaying ? 'none' : 'block', zIndex: 2 }}
        ...
        >
          <VideoStyled
            ...
            className={isPlaying && fullScreen && !mouseMove ? 'mouseHide' : ''}
            style={{
              height: videoHeight,
              width: videoWidth,
          ...
          <VideoIconStyled>
            <PlayerIconStyled
              ...
              style={{ display: isPlaying ? 'none' : 'block', zIndex: 12 }}
              className={isPlaying ? '' : 'animate'}
              onClick={videoControl}
            />
          </VideoIconStyled>
        ...
        <VideoControlsStyled
          className={isPlaying && mouseMove ? 'animate' : ''}
          ref={controls}
        >
          <ProgressContainerStyled>
            <ProgressStyled
          ...
          <ControlsStyled>
            ...
            />
            <ButtonStyled src="/images/control-settings.png" />
          </ControlsStyled>
        </VideoControlsStyled>
      </DiagonalDivStyled>
    </SectionStyled>
